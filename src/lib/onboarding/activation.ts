 ...
 *
 * NOTE, since Security Release B: `emitAuditEvent` no longer lets a caller
 * assert attribution. `actor`, `orgId`, `actorEmail` and `actorName` are still
 * accepted for source compatibility but are IGNORED — `record_audit_event()`
 * derives `actor_id` from `auth.uid()` and `org_id` from the caller's current
 * organization. They are still passed below because the signature takes them,
 * and they remain correct for this call site, but they no longer control the
 * row that gets written.
 *
 * That makes `ctx.orgId` load-bearing in one specific way worth stating: the
 * write lands in `current_org_id()` while `hasMilestone` reads back filtered by
 * `ctx.orgId`. Those are the same organization for every current caller — this
 * is always marked for the acting user in the org they are standing in — but if
 * a caller ever passes some other org, the milestone would be written once per
 * call and never found again. Callers must pass the caller's current org.
 *
