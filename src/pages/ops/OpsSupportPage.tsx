...
const SEVERITY_CLS: Record<string, string> = {
  low: 'text-gray-500 bg-gray-100 dark:text-gray-400 dark:bg-gray-800',
...
const STATUS_CLS: Record<string, string> = {
  ...
  closed: 'bg-gray-100 text-gray-500 dark:text-gray-400 dark:bg-gray-800',
  wont_fix: 'bg-gray-100 text-gray-500 dark:text-gray-400 dark:bg-gray-800',
}
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-6">
      ...
      <div className="flex gap-1 border-b border-gray-200 dark:border-gray-700">
        <button
          onClick={() => setTab('bugs')}
          className={clsx('flex items-center gap-1.5 px-4 py-2 text-sm font-medium border-b-2 -mb-px transition-colors', tab === 'bugs' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 dark:hover:text-gray-200 dark:text-gray-400')}
        >
        ...
        <button
          ...
          className={clsx('flex items-center gap-1.5 px-4 py-2 text-sm font-medium border-b-2 -mb-px transition-colors', tab === 'morph' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 dark:hover:text-gray-200 dark:text-gray-400')}
        >
      ...
      {tab === 'bugs' && (
        <div className="space-y-4">
          ...
          {bugsLoading ? (
            ...
            <div className="text-center py-8 bg-white rounded-xl border border-gray-200 dark:border-gray-700 dark:bg-gray-800">
              <CheckCircle2 className="w-8 h-8 text-green-400 mx-auto mb-2" />
              <p className="text-sm text-gray-500 dark:text-gray-400">No {statusFilter === 'all' ? '' : statusFilter} bug reports</p>
            </div>
          ) : (
                ...
                return (
                  <div key={bug.id} className="px-5 py-4 space-y-2">
                    ...
                    <div className="flex gap-1.5">
                      ...
                      {bug.status !== 'closed' && (
                        <button onClick={() => updateBugStatus.mutate({ id: bug.id, status: 'closed' })} className="px-2 py-1 text-[10px] font-medium rounded border border-gray-200 text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-800 dark:border-gray-700 dark:text-gray-400">Close</button>
                      )}
                    </div>
                  </div>
                )
      ...
      {tab === 'morph' && (
        <div className="space-y-6">
          {/* Start new morph */}
          <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-3 dark:border-gray-700 dark:bg-gray-800">
            <h3 className="text-sm font-semibold text-gray-800 dark:text-gray-100">Start Morph Session</h3>
            <div className="relative max-w-sm">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-orange-500 focus:border-transparent dark:border-gray-600"
              />
            </div>
            {morphUsers.length > 0 && (
          ...
          <div className="space-y-3">
            <h3 className="text-sm font-semibold text-gray-800 dark:text-gray-100">Session History</h3>
