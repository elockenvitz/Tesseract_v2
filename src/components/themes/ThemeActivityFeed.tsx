...
function actionText(ev: ThemeActivityEvent): React.ReactNode {
  ...
  switch (ev.activity_type) {
    ...
    case 'theme.renamed':
      return <>renamed to <span className="font-medium text-gray-900 dark:text-white">{md.new || '—'}</span></>
    ...
    case 'theme.lifecycle_changed': {
      ...
      return <>set status to <span className="font-medium text-gray-900 dark:text-white">{next}</span></>
    }
    case 'theme.archived':
    ...
    case 'theme.asset_added':
      return <>added <span className="font-medium text-gray-900 dark:text-white">{md.symbol || 'an asset'}</span></>
    case 'theme.asset_removed':
      return <>removed <span className="font-medium text-gray-900 dark:text-white">{md.symbol || 'an asset'}</span></>
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg overflow-hidden dark:border-gray-700 dark:bg-gray-800">
      <button
        ...
        className="w-full flex items-center justify-between px-4 py-2.5 bg-gray-50 border-b border-gray-200 hover:bg-gray-100 transition-colors dark:hover:bg-gray-700 dark:border-gray-700 dark:bg-gray-900"
      >
        ...
        {expanded ? <ChevronUp className="w-4 h-4 text-gray-500 dark:text-gray-400" /> : <ChevronDown className="w-4 h-4 text-gray-500 dark:text-gray-400" />}
      </button>
        ...
        <div className="p-4">
          {isLoading ? (
            <div className="space-y-2 animate-pulse">
              {[...Array(3)].map((_, i) => <div key={i} className="h-8 bg-gray-100 rounded dark:bg-gray-800" />)}
            </div>
          ) : events.length === 0 ? (
                  ...
                  <li key={ev.id} className="flex items-start gap-2.5 text-sm">
                    ...
                    <div className="flex-1 min-w-0">
                      <span className="text-gray-700 dark:text-gray-300">
                        <span className="font-medium text-gray-900 dark:text-white">{actorName(ev)}</span>{' '}
                        {actionText(ev)}
                      </span>
