...
describe('Decision rationale on a phone', () => {
  ...
  it('saves to trade_batches.description and shows the text with Edit on its own row', async () => {
    ...
    await waitFor(() => expect(screen.queryByLabelText('Why this decision?')).toBeNull())
    void container
  })
  ...
  it('reopens the same editor with the current text when editing', () => {
    ...
    expect((screen.getByLabelText('Why this decision?') as HTMLTextAreaElement).value).toBe('Adding on weakness ahead of the print.')
  ...
  it('keeps the editor open and says why when the save is refused', async () => {
    ...
    wrap(<BatchRationaleEditor batch={batch(null)} />)
    fireEvent.click(screen.getByText('Explain why you made this decision'))
    fireEvent.change(screen.getByLabelText('Why this decision?'), { target: { value: 'x' } })
    fireEvent.click(screen.getByRole('button', { name: /Save rationale/ }))
    await screen.findByText("Couldn't save rationale — permission denied or batch not found.")
    expect(screen.getByLabelText('Why this decision?')).toBeTruthy()
  })
})
...
describe('Trade rationale notes on a phone', () => {
  ...
  it('adds the note through the same handler and clears the field', () => {
    ...
    expect((screen.getByLabelText('Add a trade-specific note') as HTMLTextAreaElement).value).toBe('')
  })
  ...
  it('shows an inherited batch rationale as inherited and does not ask for it again', () => {
    ...
    const initial = container.querySelector('[data-slot="trade-rationale-initial"]') as HTMLElement
    // Points back at the batch question it came from, by that question's name.
    expect(initial.textContent).toContain('From “Why this decision?”')
    ...
    expect(screen.getByLabelText('Add a trade-specific note').getAttribute('placeholder')).not.toMatch(/batch|thesis/i)
  })
})
