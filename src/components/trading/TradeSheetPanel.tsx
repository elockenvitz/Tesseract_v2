...
function SheetListItem({
  ...
  // Fetch symbols for snapshot asset_ids missing from the passed-in map
  const missingAssetIds = useMemo(() => {
    return [...new Set(
      (variants as any[]).map((v: any) => v.asset_id).filter((id: string) => id && !assetSymbolMap[id])
    )]
  }, [variants, assetSymbolMap])

  const { data: fetchedSymbols } = useQuery({
    queryKey: ['snapshot-asset-symbols', missingAssetIds],
    queryFn: async () => {
      if (missingAssetIds.length === 0) return {}
      const { data } = await supabase.from('assets').select('id, symbol').in('id', missingAssetIds)
      const map: Record<string, string> = {}
      for (const a of data || []) map[a.id] = a.symbol
      return map
    },
    enabled: missingAssetIds.length > 0,
    staleTime: Infinity,
  })

  // Merged map: passed-in + fetched
  const symbolMap = useMemo(() => ({ ...assetSymbolMap, ...fetchedSymbols }), [assetSymbolMap, fetchedSymbols])
          ...
          <table className="w-full">
            ...
            <tbody>
              {variants.map((v: IntentVariant) => (
                <TradeRow key={v.id} variant={v} symbol={(v as any).asset?.symbol || symbolMap[v.asset_id] || '...'} />
              ))}
            </tbody>
            <tfoot>
