 ...
 *
 * Quick-launch wrapper for creating a recommendation from the Ideas pane.
 * Step 1: Select a trade idea from the current context.
 * Step 2 (embedded): Opens inline RecommendationReviewPane.
 * Step 2 (modal):    Opens the existing RecommendationEditorModal.
 */
...
import { RecommendationEditorModal } from '../trading/RecommendationEditorModal'
...
interface DisplayRow {
  key: string
  isPair: boolean
  /** The first leg (used when selecting for RecommendationEditorModal) */
...
export function RecommendationQuickModal({ isOpen, onClose, context, embedded = false }: RecommendationQuickModalProps) {
  ...
  if (selectedIdea) {
    ...
    // Modal mode → full RecommendationEditorModal
    return (
      ...
      <div className={embedded ? 'mb-2' : 'px-5 py-3 border-b border-gray-100 dark:border-gray-700'}>
        {!embedded && (
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-2">
            Select a trade idea to build your recommendation on.
          </p>
        )}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-lg w-full mx-4 max-h-viewport-80 flex flex-col">
        {/* Header */}
