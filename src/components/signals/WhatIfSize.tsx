...
interface WhatIfSizeProps {
   ...
   * Receives the proposed weight; the caller decides what staging means.
   */
 ...
 *
 * Writes must be possible but never accidental. A drag that stages a trade the
 * moment your thumb lifts is a surface nobody can explore, because every
 ...
 *
 * The slider snaps back to the current weight after staging, so the card never
...
export function WhatIfSize({
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col justify-center gap-2" data-testid="what-if-size">
      ...
      <button
        ...
        // Deliberately not onClick. A tap must never stage anything.
        className={clsx(
      ...
      >
        ...
        <span className="relative">
          {changed ? `Hold to stage ${proposed.toFixed(2)}%` : 'Drag to explore a size'}
        </span>
      </button>
    </div>
  )
