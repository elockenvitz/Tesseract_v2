/**
 * PortfolioDecisionView — Portfolio page "Needs Attention" + "Radar" surface.
 *
...
interface PortfolioDecisionViewProps {
  portfolioId: string
}

export function PortfolioDecisionView({ portfolioId }: PortfolioDecisionViewProps) {
  const { user } = useAuth()
  const userId = user?.id ?? ''
  const { selectForPortfolio, isLoading } = useDecisionEngine()

  // Slice from engine
  const { action: rawAction, intel: rawIntel } = selectForPortfolio(portfolioId)

  // Intel dismissal (shared localStorage with dashboard + asset)
