      ...
      <div className="space-y-4">
        {!embedded && (
          <div className="flex items-center gap-3">
            <button onClick={onBack} className="p-1 rounded-md hover:bg-gray-100 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 dark:hover:bg-gray-700">
              <ArrowLeft className="w-4 h-4" />
            </button>
            <h3 className="text-[15px] font-semibold text-gray-900 dark:text-white">{processName}</h3>
          </div>
        )}
        <p className="text-[12px] text-gray-500 dark:text-gray-400">Select a portfolio to view its progress.</p>
        <div className="space-y-1.5">
          {portfoliosInProcess.map(p => (
            <button
              key={p.id}
              onClick={() => setSelectedPortfolioId(p.id)}
              className="w-full flex items-center justify-between px-3 py-2.5 rounded-lg border border-gray-200 hover:border-gray-300 bg-white transition-colors text-left dark:border-gray-700 dark:bg-gray-800"
            >
              <div>
                <span className="text-[13px] font-medium text-gray-900 dark:text-white">{p.name}</span>
  ...
  if (stagesLoading || !resolvedWorkflowId) {
    return (
      <div className="space-y-6 animate-pulse">
        <Card><div className="h-[130px] bg-gray-50 rounded dark:bg-gray-900" /></Card>
        <Card><div className="h-[200px] bg-gray-50 rounded dark:bg-gray-900" /></Card>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      ...
      {!embedded && (
        <div className="flex items-center gap-3">
          ...
          }} className="p-1 rounded-md hover:bg-gray-100 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 dark:hover:bg-gray-700">
            <ArrowLeft className="w-4 h-4" />
          </button>
          <div className="flex-1 min-w-0">
            <h3 className="text-[15px] font-semibold text-gray-900 truncate dark:text-white">{processName}</h3>
          </div>
        </div>
      )}
          ...
          <div className="flex items-center gap-0.5 w-full">
            {timelineStages.map((stage, idx) => {
              ...
              const colorSchemes: Record<string, { bg: string; text: string; accent: string; progressTrack: string; progressFill: string }> = {
                completed: { bg: 'bg-emerald-500', text: 'text-white', accent: 'text-emerald-50', progressTrack: 'bg-emerald-700/30', progressFill: 'bg-white dark:bg-gray-800' },
                current: { bg: 'bg-gradient-to-br from-blue-600 to-indigo-600', text: 'text-white', accent: 'text-blue-50', progressTrack: 'bg-white/20', progressFill: 'bg-white dark:bg-gray-800' },
                    ...
                    <div className="relative z-10 space-y-2.5 min-h-[85px] flex flex-col justify-center min-w-0 w-full" style={{ marginLeft: isFirst ? '0' : '16px' }}>
                      <div className="flex items-center min-w-0">
                        <div className="relative flex items-center justify-center w-7 h-7 rounded-md bg-black/15 text-sm font-bold flex-shrink-0 mr-2">
                          ...
                          {status === 'current' && <div className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-white rounded-full animate-pulse shadow-sm dark:bg-gray-800" />}
                        </div>
      ...
      {!progress && !isCompleted && (
        <Card>
          <div className="text-center py-8">
            <p className="text-sm text-gray-500 mb-3 dark:text-gray-400">This process hasn't been started yet.</p>
            <button
          ...
          <div className="flex items-start justify-between mb-4">
            <div className="flex items-center space-x-3">
              ...
              <div className="flex-1">
                <h4 className="text-lg font-semibold text-gray-900 dark:text-white">{activeStage.stage_label} Stage</h4>
          ...
          <Card className="transition-all duration-300 animate-in fade-in slide-in-from-top-4">
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h4 className="text-lg font-semibold text-gray-900 dark:text-white">Completed</h4>
                  <p className="text-sm text-gray-600 dark:text-gray-400">
              ...
              <div>
                ...
                <div className="space-y-4">
                  {findings.map(({ item: fi, stageLabel, entries }) => (
                    <div key={fi.id}>
                      <p className="text-[11px] font-medium text-gray-500 mb-1 dark:text-gray-400">{fi.item_text} <span className="text-gray-400 font-normal">· {stageLabel}</span></p>
