      ...
      <div className="p-5 flex-1">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-2">
            ...
            <h3 className="text-sm font-semibold text-gray-900 dark:text-white">Process Definition</h3>
          </div>
          ...
          <div className="space-y-3">
            {activeVersion ? (
              <>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    {displayDescription && (
                      <p className="text-xs text-gray-500 mt-1 dark:text-gray-400">{displayDescription}</p>
                    )}
                  </div>
                </div>
                <button
                ...
                {expanded && stages && stages.length > 0 && (
                  <div className="mt-3 space-y-1">
                    {stages.map((stage, idx) => {
                      ...
                      return (
                        <div key={stage.id} className="rounded-md border border-gray-100 bg-gray-50/50 dark:border-gray-800">
                          <div className="flex items-center space-x-2 px-3 py-2">
                            <span className="text-[10px] font-medium text-gray-400 w-4 text-right">{idx + 1}</span>
                            <span className="text-sm font-medium text-gray-800 dark:text-gray-100">{stage.stage_label}</span>
                            ...
                            <div className="px-3 pb-2 space-y-0.5">
                              {stageChecklist.map(item => (
                                <div key={item.id} className="flex items-center space-x-2 pl-6">
                                  ...
                                  <span className="text-xs text-gray-500 dark:text-gray-400">{item.item_text}</span>
                                </div>
                              ))}
                            </div>
                          )}
              ...
              <div className="text-center py-4">
                <p className="text-sm text-gray-500 dark:text-gray-400">No active version</p>
              </div>
            )}
          </div>
        ) : (
          <div className="text-center py-4">
            <p className="text-sm text-gray-500 mb-2 dark:text-gray-400">No versions created yet</p>
