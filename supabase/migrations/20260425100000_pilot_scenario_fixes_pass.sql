-- Pilot scenario fixes pass:
-- 1. AAPL recommendation always reads as a BUY in pilot UX, even when
--    the demo portfolio already holds AAPL. The seeding RPC was emitting
--    'add' whenever current_weight >= 2, which broke the pilot copy
--    "BUY AAPL" the dashboard, recommendation card, and decision inbox
--    all assume.
-- 2. Decision Inbox shows a misleading "0% → 7.75%" because the seeded
--    decision_requests row has submission_snapshot = NULL. The Inbox row
--    falls back to baseline_weight=0 in that case. Backfill the snapshot
--    so the row reads "7.2% → 7.75%" (the actual sizing the trade
--    proposal already carries in sizing_context.current_weight_pct).
-- 3. The seeded MSFT secondary idea is unmovable for pilot users because
--    it's owned by the org admin (created_by = author_id) and assigned
--    to nobody. The trade_queue_items UPDATE policy allows changes only
--    when the caller is created_by, assigned_to, or PM — none of which
--    apply for a pilot user dragging a card. Assign the MSFT idea to the
--    pilot user so they can move it through the kanban.
--
-- All three changes are backfills + an RPC update so future pilot seeds
-- bake the correct values from the start.

-- ────────────────────────────────────────────────────────────────────
-- 1+3. Backfill existing pilot data
-- ────────────────────────────────────────────────────────────────────

-- AAPL: flip residual 'add' actions to 'buy' on the recommendation TQI.
UPDATE trade_queue_items tqi
SET action = 'buy'::trade_action
WHERE origin_metadata->>'role' = 'recommendation'
  AND origin_metadata ? 'pilot_scenario_id'
  AND action = 'add'::trade_action;

-- AAPL: align pilot_scenarios direction + proposed_action label so the
-- dashboard reads BUY everywhere.
UPDATE pilot_scenarios
SET direction = 'buy',
    proposed_action = CASE
      WHEN proposed_action ILIKE 'Add to existing position%' THEN 'Buy AAPL'
...
-- AAPL: align pending decision_requests' requested_action.
UPDATE decision_requests dr
SET requested_action = 'buy'
...
WHERE dr.trade_queue_item_id = tqi.id
  ...
  AND dr.requested_action = 'add'
  AND dr.status = 'pending';

-- 2. Decision Inbox snapshot backfill — populate submission_snapshot
--    with baseline_weight so the row renders "7.2% → 7.75%" instead of
--    the misleading "0% → 7.75%".
UPDATE decision_requests dr
SET submission_snapshot = jsonb_build_object(
      'baseline_weight', COALESCE(
        (tp.sizing_context->>'current_weight_pct')::NUMERIC,
        0
      ),
      'weight', dr.sizing_weight,
      'sizing_context', tp.sizing_context
    )
FROM trade_queue_items tqi
JOIN trade_proposals tp
  ON tp.trade_queue_item_id = tqi.id
 AND tp.is_active = TRUE
WHERE dr.trade_queue_item_id = tqi.id
  AND tqi.origin_metadata->>'role' = 'recommendation'
  AND tqi.origin_metadata ? 'pilot_scenario_id'
  AND dr.status = 'pending'
  AND dr.submission_snapshot IS NULL;

-- 3. MSFT (and any other unassigned pilot 'idea' role) → assign to the
--    pilot user that owns the scenario so RLS lets them update stage.
UPDATE trade_queue_items tqi
SET assigned_to = ps.user_id
FROM pilot_scenarios ps
WHERE tqi.origin_metadata ? 'pilot_scenario_id'
  AND (tqi.origin_metadata->>'pilot_scenario_id')::UUID = ps.id
  AND tqi.assigned_to IS NULL
  AND ps.user_id IS NOT NULL
  AND ps.is_template = FALSE
  AND ps.status = 'active';

-- ────────────────────────────────────────────────────────────────────
-- Forward fix: ensure_pilot_scenario_for_user — keep AAPL action as
-- BUY, assign secondary MSFT idea to the pilot user.
-- ────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- Pilot UX rule: the recommendation always reads as a BUY regardless of
  -- whether the demo portfolio already holds the asset. The math (target,
  -- delta) still reflects reality so the inbox / lab show real numbers.
...
-- ────────────────────────────────────────────────────────────────────
-- Forward fix: ensure_pilot_decision_request_for_user — populate
-- submission_snapshot so the Decision Inbox renders the correct
-- baseline weight, and prefer 'buy' as the requested_action.
-- ────────────────────────────────────────────────────────────────────
