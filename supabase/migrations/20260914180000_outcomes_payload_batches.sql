-- outcomes_payload: carry each accepted trade's batch.
--
-- Outcomes groups decisions by the batch they were committed in. The payload
-- already returns each decision's active accepted trades (step 6); this adds
-- three fields to those rows:
--
--   batch_id          accepted_trades.batch_id
--   batch_name        trade_batches.name
--   batch_created_at  trade_batches.created_at  (the commit date)
--
-- Nothing else in the function changes. The body below is the LIVE production
-- definition as read on 2026-09-14 (pg_get_functiondef), not the older
-- 20260426130000 migration file, which no longer matches production.
--
-- ── Access ────────────────────────────────────────────────────────────────
-- The function stays SECURITY INVOKER (no SECURITY DEFINER), so every read,
-- including the new join, runs under the caller's RLS:
--   accepted_trades  SELECT "Accepted trades: org-scoped": portfolio in the
--                    current org AND (portfolio member OR org admin).
--   trade_batches    SELECT "Trade batches: members or org admins view":
--                    portfolio member OR (current org AND org admin).
-- A trade the caller can see is in a portfolio they are a member of, or an
-- org-admin portfolio in their current org; either way its batch (same
-- portfolio — 0 mismatches in production) passes the trade_batches policy.
-- If a batch row were ever hidden, the LEFT JOIN yields null name/date and
-- only batch_id — already readable on accepted_trades — is returned.
-- No policy, grant or table is changed.
...
BEGIN
  ...
  -- Status list mirrors the client logic: default to approved+executed,
  -- include rejected/cancelled when their flag is on.
  ...
  -- Step 2: Events (last 180d, scoped to decisions' portfolios)
  ...
  -- Step 3: Rationales (latest per event)
  ...
  -- Step 5: Decision-time price snapshots (approval-only)
  ...
  -- Step 6: Accepted trades + note rollup (note_count, latest_note)
  --         + the batch each was committed in (batch_id, batch_name, batch_created_at)
  IF v_decision_ids IS NOT NULL AND array_length(v_decision_ids, 1) > 0 THEN
    WITH at AS (
      SELECT
        ...
        tb.created_at AS batch_created_at,
      ...
      LEFT JOIN trade_batches tb ON tb.id = at.batch_id
      WHERE at.trade_queue_item_id = ANY(v_decision_ids)
        AND at.is_active = TRUE
    )
    SELECT jsonb_agg(to_jsonb(at.*))
    INTO v_accepted_trades
    FROM at;
  END IF;
