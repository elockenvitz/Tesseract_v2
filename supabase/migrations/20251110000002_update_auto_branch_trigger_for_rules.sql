-- Update the auto-create branch function to work with automation rules instead of workflow fields
  ...
  IF v_auto_create_rule.id IS NOT NULL THEN
    ...
    IF v_branch_suffix IS NOT NULL AND v_branch_suffix != '' THEN
      ...
      IF v_branch_count = 0 THEN
        ...
        RAISE NOTICE 'Auto-created workflow branch: % (ID: %) via automation rule', v_generated_name, v_new_branch_id;
      END IF;
    END IF;
  END IF;
...
COMMENT ON FUNCTION auto_create_workflow_branch() IS 'Automatically creates a workflow branch when the first asset is assigned to a workflow with an active auto_create_branch automation rule';
