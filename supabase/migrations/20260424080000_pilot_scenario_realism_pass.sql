/**
 * Make pilot-seeded content read like a real decision.
 *
 *   1. AUTHOR: the Recommendation should look like it came from someone
 *      other than the pilot user (self-recommendations are weird). Pick
 *      the first org admin who isn't the pilot; else any platform admin;
 *      else the caller; last resort the pilot themselves.
 *
 *   2. TARGET RELATIVE TO HOLDINGS: if the default asset is already in
 *      the portfolio, a flat "target 2%" can silently imply a TRIM. We
 *      now read the latest portfolio_holdings_snapshots row and compute
 *      action / target / delta relative to the current weight:
 *        not held   → buy,  target 2%,          delta +2%
 *        held < 2   → add,  target 2%,          delta +(2 − current)
 *        held ≥ 2   → add,  target current+0.5, delta +0.5%
 *
 *   3. SECONDARY IDEA: also stage a plain trade_queue_items row (MSFT,
 *      thesis_forming, no proposal) so the Ideas section isn't empty.
 *      Ancillary items are tagged in origin_metadata so Reset can clean
 *      them up.
 *
 * Idempotency, advisory lock, and ops-panel gates are unchanged.
 */
...
BEGIN
  ...
  -- Reset path: archive scenarios + soft-delete every queue item tagged
  -- to one of those scenarios (covers the Recommendation AND the bonus
  -- Ideas-section item).
  ...
  -- Author resolution (never the pilot themselves if we can help it).
  ...
  IF v_author_id IS NULL THEN
    ...
    WHERE pa.user_id <> v_target_user
    ORDER BY pa.user_id
    LIMIT 1;
  END IF;
  ...
  -- Current-weight lookup and action/target reframing.
  ...
  -- Override default target ONLY when template didn't specify one.
  IF v_template.target_weight_pct IS NULL THEN
    ...
    ELSIF v_current_weight < 2 THEN
      ...
      v_proposed_action := format('Add to existing position (%.1f%% → 2.0%%)', v_current_weight);
      v_proposed_sizing := format('+%.1f', 2 - v_current_weight);
    ELSE
      v_direction := 'add';
      v_target_weight := v_current_weight + 0.5;
      v_delta_weight := 0.5;
      v_proposed_action := format('Add to existing position (%.1f%% → %.1f%%)', v_current_weight, v_current_weight + 0.5);
  ...
  -- Scenario row first — we need scenario_id for the origin_metadata tag.
  ...
  IF v_portfolio_id IS NOT NULL AND v_asset_id IS NOT NULL THEN
    ...
    v_proposal_notes := v_proposed_action
      || CASE WHEN v_current_weight > 0
              THEN format(E'.\n\nCurrent weight: %.1f%%. Delta: %+.1f%%.',
                          v_current_weight, v_delta_weight)
              ELSE format(E'.\n\nTarget weight: %.1f%%. Delta: %+.1f%%.',
                          v_target_weight, v_delta_weight)
         END
    ...
    -- Secondary bare idea (MSFT, thesis_forming, no proposal).
...
BEGIN
  ...
  UPDATE trade_proposals SET is_active = FALSE, updated_at = now()
    WHERE trade_queue_item_id IN (
      SELECT id FROM trade_queue_items
      WHERE origin_metadata->>'pilot_scenario_id' = p_scenario_id::text
    ) AND is_active = TRUE;
