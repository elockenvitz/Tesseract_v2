/**
 * Use live portfolio_holdings (shares x price) instead of stale snapshots.
 *
 * Prior revisions read from `portfolio_holdings_positions`, which is a
 * history table driven by uploaded snapshots. For pilot orgs whose live
 * holdings have diverged from the last snapshot, the "current weight" the
 * seeding RPC reads could be wildly wrong — e.g. 4% from an old snapshot
 * when the live portfolio actually holds AAPL at 7.32%. The recommendation
 * target (current + 0.5%) was then nonsensical relative to what the pilot
 * user sees in the sim.
 *
 * Live weight = shares * price / sum(shares * price) across the portfolio.
 * Same shape as what the frontend's own weight pct column computes.
 *
 * Also: MSFT bonus idea now sets proposed_weight=NULL so it doesn't read
 * as an implicit recommendation in the left panel.
 */
...
BEGIN
  ...
  -- Live weight from portfolio_holdings, not the snapshot table. The
  -- snapshot can be months stale; the live table is what the front end
  -- computes weights against.
  ...
  IF v_portfolio_id IS NOT NULL AND v_asset_id IS NOT NULL THEN
    ...
    -- Bonus bare idea: MSFT thesis-forming, no proposed_weight so it
    -- doesn't masquerade as a recommendation.
