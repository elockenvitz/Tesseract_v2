-- Suppress pilot seeding during an active morph session.
--
-- Background: when a platform admin morphs into a pilot user,
-- morph_switch_org() temporarily swaps the admin's
-- users.current_organization_id to the target's pilot org. The admin's
-- auth.uid() is unchanged. Frontend surfaces (Trade Lab, dashboard)
-- then call ensure_pilot_scenario_for_user() which inspects the admin's
-- current_organization_id, sees a pilot org, and seeds a new
-- pilot_scenario + demo trade_queue_items WITH THE ADMIN'S user_id —
-- enrolling the admin as a pilot in an org they're not a member of and
-- polluting that org's Trade Lab with the admin's seeded ideas.
--
-- Already happened to elockenvitz@yahoo.com (platform admin) in Helgans
-- Growth Capital and Metz Capital. Cleaned up in companion migration.
--
-- Fix: short-circuit the two seeders when the caller has an active
-- morph session AND is self-seeding (p_user_id NULL or = auth.uid()).
-- Explicit admin "seed for user X" calls from the ops panel still work
-- because they pass a non-self p_user_id.

CREATE OR REPLACE FUNCTION public.has_active_morph_session()
RETURNS BOOLEAN
LANGUAGE sql STABLE SECURITY DEFINER
SET search_path = public
AS $$
  SELECT EXISTS (
    SELECT 1 FROM morph_sessions
...
GRANT EXECUTE ON FUNCTION public.has_active_morph_session() TO authenticated;

-- Patch ensure_pilot_scenario_for_user: add a self-seed-during-morph guard.
-- Body is identical to the prior definition except for the new IF block
-- right after the existing access checks.
...
BEGIN
  ...
  -- Morph guard: if the caller is self-seeding while morphed into another
  -- user, no-op. Their current_organization_id is temporarily pointing at
  -- the morph target's org (which may be a pilot org); seeding here would
  -- enroll the admin as a pilot there.
  IF (p_user_id IS NULL OR p_user_id = v_caller_id) AND has_active_morph_session() THEN
    RETURN jsonb_build_object('seeded', FALSE, 'reason', 'morph_session_active');
  END IF;
...
-- Patch seed_pilot_pipeline_demo_ideas: add the same morph guard.
-- Body otherwise unchanged from the race-fix migration.
...
BEGIN
  ...
  -- Morph guard: see ensure_pilot_scenario_for_user above.
  IF (p_user_id = v_caller_id) AND has_active_morph_session() THEN
    RETURN 0;
  END IF;
