-- Pilot users keep tripping over portfolio_membership-gated RLS:
-- decision_requests, accepted_trades, trade_idea_portfolios, trade_labs,
-- lab_variants, trade_batches, portfolio_trade_events, portfolio_holdings,
-- accepted_trade_comments, trade_idea_theses (plus React filters that
-- gate on portfolio_memberships.has(user.id) for portfolio context).
-- Each gate has had to be patched individually with an org-admin path.
--
-- Real fix: pilot users SHOULD be portfolio members of their pilot
-- portfolio. Granting them is_portfolio_manager=true means every
-- portfolio-member RLS policy and every "is the user a PM?" React
-- check resolves correctly — no per-table broadening required for
-- the pilot scenario flow.
--
-- This migration:
--   1. Backfills portfolio_memberships for every (user_id, portfolio_id)
--      pair from active pilot_scenarios.
--   2. Patches ensure_pilot_scenario_for_user to seed the membership
--      going forward, and to heal pre-migration pilots on next login
...
-- The patched function body is identical to the previous version
-- except for two new INSERT...ON CONFLICT blocks that ensure the
-- portfolio_memberships row exists. See the inline comments below
-- for "NEW:" markers.
...
BEGIN
  ...
  IF FOUND THEN
    -- NEW: heal pre-migration pilots on their next login.
  ...
  -- NEW: enroll the pilot user as portfolio manager so every
  -- portfolio_member-gated policy resolves without per-table fixes.
