/**
 * Pilot mode becomes org-scoped.
 *
 * Before: pilot-ness = users.is_pilot_user OR org.settings.pilot_mode. A
 * user marked as a pilot for client org A would stay in pilot mode after
 * switching to their own non-pilot org B, with A's staged scenario leaking
 * into B (pilot_scenarios fetches weren't org-filtered).
 *
 * After: pilot-ness is driven exclusively by the current org's
 * settings.pilot_mode. The users.is_pilot_user column is left in place
 * (no data loss) but no longer read anywhere.
 *
 * This migration updates two server-side consumers that still checked the
 * per-user flag:
 *   - is_current_pilot_session()   — drops the user-level OR
 *   - ensure_pilot_scenario_for_user() — seeds based on org.pilot_mode only
 */
...
AS $$
  SELECT COALESCE(
    (
      ...
      WHERE o.id = (SELECT u.current_organization_id FROM public.users u WHERE u.id = auth.uid())
    ),
    false
  );
$$;
...
BEGIN
  ...
  -- Scope the "already exists" check to this org so a scenario staged in
  -- one pilot org doesn't block seeding in another.
