...
CREATE POLICY asset_field_history_select ON public.asset_field_history
  ...
  USING (
    ...
    field_name IN ('process_stage', 'priority', 'thesis_references')
...
DO $$
DECLARE system_rows int; research_rows int; orphaned int;
BEGIN
  SELECT count(*) FILTER (WHERE field_name IN ('process_stage','priority','thesis_references')),
         count(*) FILTER (WHERE field_name IN ('thesis','risks_to_thesis','where_different')),
         count(*) FILTER (WHERE field_name IN ('thesis','risks_to_thesis','where_different')
